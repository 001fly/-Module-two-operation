...
bracket = re.compile(r'\([^()]+\)')  # 寻找最内层括号规则(括号内不包含括号)
mul = re.compile(r'(-?\d+\.?\d*\*-?\d+\.?\d*)')  # 寻找乘法运算规则  23.43 * -32.432
div = re.compile(r'(-?\d+\.?\d*/-?\d+\.?\d*)')  # 寻找除法运算规则   23.43 / -32.432
add = re.compile(r'(-?\d+\.?\d*\+-?\d+\.?\d*)')  # 寻找加法运算规则  23.43 + -32.432
sub = re.compile(r'(-?\d+\.?\d*--?\d+\.?\d*)')  # 寻找减法运算规则  23.43 - -32.432
letter = re.compile(r'[A-Za-z]')  # 检查表达式是否含有字母规则
...
def calc():
    while True:
        ...
        if s == 'q':
            break
        elif letter.findall(s) == []:   # 判断表达式是否包含字母
            ...
            print('The answer is: %.3f' % (float(s)))
        else:
            print("The input expression is illegal. Please check again!")
